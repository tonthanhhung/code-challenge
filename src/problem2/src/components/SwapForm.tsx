...
export function SwapForm({ tokens }: Props) {
  ...
  function validate() {
    ...
    if (!fromToken) errs.fromToken = t('validation.selectTokenToSend');
    if (!toToken) errs.toToken = t('validation.selectTokenToReceive');
    
    const amountNum = Number(fromAmount);
    if (!fromAmount || isNaN(amountNum)) {
      errs.fromAmount = t('validation.enterValidNumber');
    } else if (amountNum <= 0) {
      errs.fromAmount = t('validation.amountGreaterThanZero');
    }
  ...
  if (status === 'success') {
    return (
      <SuccessContainer>
        ...
        <SuccessTitle>{t('swap.swapConfirmed')}</SuccessTitle>
        <SuccessText>
          <span 
            dangerouslySetInnerHTML={{
              __html: t('swap.swapDescription', { 
                amount: fromAmount, 
  ...
  return (
    <Form onSubmit={handleSubmit} noValidate>
      ...
      >
        {status === 'loading' ? (
          <Spinner aria-label={t('app.loading')} />
        ) : (
          t('swap.confirmSwap')
        )}
      </SubmitButton>
